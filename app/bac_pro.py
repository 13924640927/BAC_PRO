...
# 导入 AI 扫描器
try:
    from core.vision_scanner import BaccaratVisionScanner
except ImportError:
    BaccaratVisionScanner = None
import os
import sys
...
class CameraWindow(tk.Toplevel):
    def __init__(self, parent, ai_scanner, callback):
        super().__init__(parent)
        self.title("AI Vision - Manual Capture")
        ...
        self.ctrl_frame.pack(fill="x", side="bottom", pady=20)

        # 聚焦控制
        self.focus_var = tk.IntVar(value=0)
        ttk.Label(self.ctrl_frame, text="Focus:").pack(side="left", padx=(20, 5))
        self.focus_slider = ttk.Scale(self.ctrl_frame, from_=0, to=255, variable=self.focus_var, orient="horizontal", command=self.set_focus)
        self.focus_slider.pack(side="left", fill="x", expand=True, padx=10)

        ttk.Button(self.ctrl_frame, text="CAPTURE", command=self.capture).pack(side="left", padx=10)
        self.btn_analyze = ttk.Button(self.ctrl_frame, text="RUN AI ANALYSIS", command=self.run_analysis, state="disabled")
        self.btn_analyze.pack(side="left", padx=10)
        self.btn_retake = ttk.Button(self.ctrl_frame, text="RETAKE", command=self.retake, state="disabled")
        self.btn_retake.pack(side="left", padx=10)

        self.update_video()
        self.protocol("WM_DELETE_WINDOW", self.on_close)

    def set_focus(self, _=None):
        val = self.focus_var.get()
        self.cap.set(cv2.CAP_PROP_AUTOFOCUS, 0) # 必须先关自动对焦
    ...
    def run_analysis(self):
        if self.current_frame is not None:
            ...
            self.btn_analyze.config(text="ANALYZING...", state="disabled")
            self.update()
            data = self.ai_scanner.get_road_data(temp_path)
            if data:
                self.callback(data)
                if os.path.exists(temp_path): os.remove(temp_path)
                self.on_close()
            else:
                messagebox.showerror("Error", "AI Analysis Failed.")
                self.retake()
...
class App(tk.Tk):
    def __init__(self):
        ...
        # 在原有 Deal One 和 New Shoe 按钮下方添加：
        ttk.Button(ctrl_box, text="AI CAMERA SCAN", command=self.open_ai_camera).pack(fill="x", padx=8, pady=(0, 8))
        # --- right: SBI (STEP3) ---
        ...
        if os.path.exists(logo_path):
            try:
                ...
                img = img.resize((250, 150), RESAMPLE)
                ...
                self.logo_label.place(relx=1.0, rely=1.0, anchor="se", x=-10, y=-22)
            except Exception:
                pass
    ...
    def new_shoe(self):
        ...
        self._refresh_sbi_panel()
    def open_ai_camera(self):
            """打开 AI 拍照窗口"""
            if not hasattr(self, 'ai_scanner') or self.ai_scanner is None:
                self.ai_scanner = BaccaratVisionScanner()
            CameraWindow(self, self.ai_scanner, self.process_ai_result)

    def process_ai_result(self, road_data):
        """
        将 AI 识别的 [b1, p3...] 列表同步到 APP 引擎中，
        并自动触发您原有的 Snapshot 分析和数据库 EV 查询。
        """
        # 1. 状态提示
        self._update_status("AI Data Syncing & Querying EV...")
        
        # 2. 核心步骤：开启新局（New Shoe），确保从零开始同步
        # 这会重置您的 hist_state, streak_board 和所有计数器
        self.new_shoe() 
        
        # 3. 遍历 AI 识别出的每一条 Streak (例如 'b1', 'p3')
        for streak_str in road_data:
            if not streak_str or len(streak_str) < 2:
                continue
                
            side = streak_str[0].upper() # 转换为 'B' 或 'P'
            try:
            ...
            for i in range(count):
                # A. 更新大路图 UI (调用您原有的 StreakBoard)
                self.streak_board.push_result(side)
                
                # B. 触发原有的 PREMAX/Snapshot 分析逻辑
                # 这会执行以下您原有的流程：
                # - 在颜色切换时自动调用 self.hist_state.apply_streak_to_history
                # - 自动构建 state_key
                # - 自动调用 query_premax_ev 查询数据库
                # - 自动更新 self.ev_out.config(text=...) 显示结果
                self._premax_on_bp_result(side)
        
        # 5. 完成同步后的处理
        self._update_status(f"AI Sync Done: {len(road_data)} streaks processed.")
        
        # 弹出提示，告知用户 EV 已更新
        messagebox.showinfo("AI Sync Success", 
                            f"已成功加载 {len(road_data)} 条路单数据。\n"
                            "Snapshot 分析已完成，EV 结果已在界面更新。")
