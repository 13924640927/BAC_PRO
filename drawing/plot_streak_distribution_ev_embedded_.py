# plot_streak_ev_embedded.py
# All data embedded (no file I/O). Source: PY_BAC_STREAK_dist.csv
...
def plot_metric(metric: str, title: str, y_label: str):
    series = group_series(metric)
...
def main():
    plot_metric(
        metric="ev_cut_pct",
        title="STREAK LEN EV (CUT) by Side/Scope",
        y_label="EV_CUT_PCT",
    )
    plot_metric(
        metric="ev_con_pct",
        title="STREAK LEN EV (CONTINUAL) by Side/Scope",
        y_label="EV_CON_PCT",
    )
    plt.show()
